...
# 1. Input x will be [1.0, 1.0, 1.0]
# (Provide this via -input-data=1.0,1.0,1.0)

# 2. Set weight to 0 so x + weight = [1.0, 1.0, 1.0]
weight_val = np.array([0.0, 0.0, 0.0], dtype=np.float32)

# 3. Set first constant to 0
val1 = np.array([0.0, 0.0, 0.0], dtype=np.float32)

# 4. Set MatMul matrix to ALL ONES (3x2)
# This means every output = (1*1) + (1*1) + (1*1) = 3.0
val2 = np.ones((3, 2), dtype=np.float32)
